...
close all;
%%
data_folder = '/Volumes/yuan_lab/TIER2/anthracosis/never_smoker/4_cell_class/csv'; %annotations / cells
seg_folder = '/Volumes/yuan_lab/TIER2/anthracosis/never_smoker/3_cell_seg/mat'; %1054 in RDS, 947 in proj5, not sure how many is overlapped
cws_folder = '/Volumes/yuan_lab/TIER2/anthracosis/never_smoker/1_cws_tiling';
files = dir(fullfile(seg_folder, '*.svs'));

%%
save_folder = '/Volumes/yuan_lab/TIER2/anthracosis/never_smoker/cellFeatures_v1';
%save_filename = 'regional_tilesArea.csv';
if ~exist(save_folder, 'dir')
    mkdir(save_folder);
end
%%
for i = 62: length(files)
    sample = files(i).name;
    sampleName = sample; %sample(1:end-5);
    if ~isfile(fullfile(save_folder, [sampleName '.csv']))
            ...
            if ~isfile(fullfile(save_folder,sample,[DaName '.csv']))
                ...
                if isfield(seg, 'mat')
                    if any(seg.mat.BinLabel(:))
                        ...
                        class.Properties.VariableNames = {'class' 'class_x' 'class_y' 'class2'}; %V1: class, V2: class_x, V3: claa
                        stats = [class stats];
